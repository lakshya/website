...
final class Kohana {
	...
	public static function activity($query)
	{
		$user = Session::instance()->get('username');
		$filename = APPPATH."files/activity/".date('Y-m-d').'.log'.EXT;
		if ( ! is_file($filename))
		{
			file_put_contents($filename,'');
			chmod($filename, 0644);
		}
		$message = date('Y-m-d H:i:s')." --> ".$user." --> ".$query.PHP_EOL;
	...
	{
		$a = new ReflectionClass("Home_Controller");
		$b = $a->newInstance();
		$c = $a->getMethod("_error404");
		$c->invokeArgs($b, array());
