...
class form_Core {
	...
	public static function checkgroup($name, $values, $defaults = null, $newlines = FALSE)
	{
		$ret = '';
		
		foreach($values as $a=>$b)
		{
			$ret .= '<label class="checkbox"><input type = "checkbox" name = "'.$name.'" value = "'.$a.'" ';
			if($defaults != NULL && in_array($a, $defaults)) $ret .= 'checked = "checked"';
			$ret .= ' /> '.$b.'</label>';
			if($newlines) $ret .= "<br />\n";
		}
		return $ret;
	...
	public static function radiogroup($name, $values, $defaults = null, $newlines = FALSE)
	{
		$ret = '';
		
		foreach($values as $a=>$b)
		{
			$ret .= '<label class="radio"><input type = "radio" name = "'.$name.'" value = "'.$a.'" ';
			if($a == $defaults) $ret .= 'checked = "checked"';
			$ret .= ' /> '.$b.'</label>';
			if($newlines) $ret .= "<br />\n";
		}
		return $ret;
